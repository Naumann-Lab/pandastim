...
stimulus_path = Path(sys.executable).parents[0].joinpath(r'Lib\site-packages\pandastim\resources\protocols\sevenrep_twentyonestim.hdf')
...
def wrapper(protocol, stimulus_dataframe_path, ports, params, parameter_path):
    ...
    stytraBuddy = StytraBuddy(comms = ports, stimuli = None,
                             params_path =parameter_path,
                              protocol = protocol)  # start with pausing until Go, get stytra position output
    #pstim = stimulus.StimulusSequencing(buddy=stytraBuddy, params_path=parameter_path, buddy_port = ports['buddy_stimulus_socket'])
...
if __name__ == '__main__':
        ...
        stimulus_process = mp.Process(target=wrapper, args=(CenterClickTestingProtocol, None, _ports, params, parameter_path))
