...
from stytra.tracking.fisheye import FishEyeTrackingMethod
...
class FishEyeTrackingPipeline(Pipeline):
    def __init__(self):
        super().__init__()
        self.bgsub = BackgroundSubtractor(parent=self.root)
        self.fishtrack = FishEyeTrackingMethod(parent=self.bgsub)
        self.extra_widget = BoutPlot
        self.display_overlay = CameraViewFish
...
pipeline_dict = dict(
    ...
    fish_eye = FishEyeTrackingPipeline,
)
