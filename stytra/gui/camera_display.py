...
@jit(nopython=True)
def _tail_points_from_coords(coords, seglen, n_segment):
...
class CameraViewFish(CameraViewCalib):
    ...
    def retrieve_image(self):
        ...
        try:
            ...
            self.points_fish.setData(x=retrived_data[valid, 2], y=retrieved_data[valid, 0])
            if n_points_tail:
                tail_len = (
                    self.tracking_params.tail_length / self.tracking_params.n_segments
                )
                ys, xs = _tail_points_from_coords(retrieved_data, tail_len, n_points_tail)
...
class CameraViewEyeFish(CameraViewCalib):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.points_fish = pg.ScatterPlotItem(
            size=5, pxMode=True, brush=(0, 255, 0), pen=None
        )
        self.lines_fish = pg.PlotCurveItem(
            connect="pairs", pen=pg.mkPen((10, 100, 200), width=3)
        )
        self.leye_fish = pg.EllipseROI(
            pos=(0, 0), size=(10, 10), movable=False, pen=dict(color=(255, 0, 0), width=1)
        )
        self.reye_fish = pg.EllipseROI(
            pos=(0, 0), size=(10, 10), movable=False, pen=dict(color=(0, 0, 255), width=1)
        )
        self.display_area.addItem(self.points_fish)
        self.display_area.addItem(self.lines_fish)
        self.display_area.addItem(self.leye_fish)
        self.display_area.addItem(self.reye_fish)
        [self.reye_fish.removeHandle(h) for h in self.reye_fish.getHandles()]
        [self.leye_fish.removeHandle(h) for h in self.leye_fish.getHandles()]
        self.tracking_params = self.experiment.pipeline.fishtrack._params
    ...
    def retrieve_image(self):
        ...
        try:
            ...
            #    y=retrieved_data[valid, 2], x=retrieved_data[valid, 0]
            #)
            self.points_fish.setData(x=retrieved_data[valid, 2], y=retrieved_data[valid, 0])
            ...
            if n_points_tail:
                ...
                for fish in range(n_fish):
                    if len(retrieved_data[valid, -10]) > 0:
                        if retrieved_data[valid, -10][fish] == retrieved_data[valid, -10][fish]:#if not nan eye
                            try:
                                self.leye_fish.setPen((255, 0, 0), width=3)
                                self.reye_fish.setPen((0, 0, 255), width=3)

                                self.leye_fish.setPos(retrieved_data[valid, -9][fish] - self.leye_fish.size()[0] / 2,
                                                     retrieved_data[valid, -10][fish] - self.leye_fish.size()[1] / 2)
                                self.reye_fish.setPos(retrieved_data[valid, -4][fish] - self.reye_fish.size()[0] / 2,
                                                     retrieved_data[valid, -5][fish] - self.reye_fish.size()[1] / 2)
                                self.leye_fish.setSize((retrieved_data[valid, -8][fish], retrieved_data[valid, -7][fish]), center=(0.5, 0.5))
                                self.reye_fish.setSize((retrieved_data[valid, -3][fish], retrieved_data[valid, -2][fish]), center=(0.5, 0.5))
                                theta = retrieved_data[valid, 4][fish]
                                leye_center = self.leye_fish.boundingRect().center()
                                leye_transform = QTransform()
                                leye_transform.translate(leye_center.x(), leye_center.y()).rotate(-np.rad2deg(theta) + retrieved_data[valid, -6][fish] + 90).translate(-leye_center.x(), -leye_center.y())
                                self.leye_fish.setTransform(leye_transform)
                                reye_center = self.reye_fish.boundingRect().center()
                                reye_transform = QTransform()
                                reye_transform.translate(reye_center.x(), reye_center.y()).rotate(-np.rad2deg(theta) + retrieved_data[valid, -1][fish] + 90).translate(-reye_center.x(),-reye_center.y())
                                self.reye_fish.setTransform(reye_transform)
                            except ZeroDivisionError:
                                pass
                        else:
                            self.leye_fish.setPen(None)
                            self.reye_fish.setPen(None)
                    else:
                        self.leye_fish.setPen(None)
                        self.reye_fish.setPen(None)
