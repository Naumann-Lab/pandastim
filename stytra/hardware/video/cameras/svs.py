...
class SVSCamera(Camera):
    ...
    def __init__(self, downsampling=1, roi=(-1, -1, -1, -1),
        ...
        self.roi = roi
        #super(Camera, self).__init__(roi = roi, *args, **kwargs)
    ...
    def set_feature_value_int64(self, remote_device_handle, feature_handle, value):
        ...
        res = self.sv_dll.SVFeatureSetValueInt64(remote_device_handle, feature_handle, ct.c_int64(value))
        return res
    ...
    def get_imagesize(self):
        feature_handle = self.get_feature_by_name(self.remote_device_handle, 'Width')
        width = self.get_feature_value_int64(self.remote_device_handle, feature_handle)
        feature_handle = self.get_feature_by_name(self.remote_device_handle, 'Height')
        height = self.get_feature_value_int64(self.remote_device_handle, feature_handle)
        return (height, width)
    ...
    def SVUtilSaveImageToFile(self, info, file_name, file_type):
        ...
        file_name_bytes = ct.c_char_p(file_name.encode("utf-8"))
        result = self.sv_dll.SVUtilSaveImageToFile(ct.byref(info), file_name_bytes, SV_IMAGE_FILE_TYPE(file_type))
        if result != 0:
    ...
    def get_buffer_info(self, stream_handle, buffer_handle):
        ...
        self.sv_dll.SVStreamBufferGetInfo(stream_handle, buffer_handle, ct.byref(buffer_info))
        return buffer_info
    ...
    def queue_buffer(self, stream_handle, buffer_handle):
        ...
        self.sv_dll.SVStreamQueueBuffer(stream_handle, buffer_handle)
    ...
    def acquiring_imaging(self, stream_handle):
        ...
        while self.acquiring:
            self.buffer_handle, result = self.wait_for_new_buffer(stream_handle, 1000)
            if result == 0:  # New buffer received
                buffer_info = self.get_buffer_info(stream_handle, self.buffer_handle)
    ...
    def open_camera(self, buffer_count = 10):
        ...
        #set up roi [x, y, width, height]
        roi_dict = ['OffsetX', 'OffsetY', 'Width', 'Height']
        for value, name in zip(self.roi, roi_dict):
            if value >= -1:
                feature_handle = self.get_feature_by_name(self.remote_device_handle, name)
                self.set_feature_value_int64(self.remote_device_handle, feature_handle, int(value))

        #start acquisition
        self.imagesize = self.get_imagesize()
    ...
    def set(self, param, val):
        ...
        """
        if param == 'exposure':
            feature_handle = self.get_feature_by_name(self.remote_device_handle, 'ExposureTime')
            self.set_feature_value_float(self.remote_device_handle, feature_handle, val * 1000)#translate ms to s
        ...
        if self.last_buffer_info is not None:
            ...
            if (bpp == 8):
                ...
                self.image = np.ndarray(buffer=imagedata.contents, dtype=np.uint8,
                                         shape=(self.last_buffer_info.iSizeY, self.last_buffer_info.iSizeX, 1))[:, :, 0]
            elif (bpp == 12):
            ...
            self.image = np.full(self.imagesize, 255, dtype=np.uint8)#cusioning for when the camera is opening up and stytra is reading
        return self.image
