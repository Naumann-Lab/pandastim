...
def _fish_column_names(i_fish, n_segments):
    ...
    ] + ["f{:d}_pos_x_e0".format(i_fish),"f{:d}_pos_y_e0".format(i_fish),
        "f{:d}_dim_x_e0".format(i_fish),"f{:d}_dim_y_e0".format(i_fish),
         "f{:d}_th_e0".format(i_fish),
         "f{:d}_pos_x_e1".format(i_fish),"f{:d}_pos_y_e1".format(i_fish),
         "f{:d}_dim_x_e1".format(i_fish),"f{:d}_dim_y_e1".format(i_fish),
        "f{:d}_th_e1".format(i_fish)]


class FishEyeTrackingMethod(ImageToDataNode):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, name="fisheye_tracking", **kwargs)
    ...
    def _eyeprocess(self, im, theta, border_margin,
        ...
        ** extraparams
        ...
        """
        heavily contribution from original Stytra code and ChatGPT
        Parameters
        ----------
        eye_im :
            image (numpy array);
        ...
        (h, w) = im_up.shape[:2]
        cos, sin = abs(np.cos(theta)), abs(np.sin(theta))#make sure the rotated thing didn't get cropped
        new_w = int(h * sin + w * cos)
        new_h = int(h * cos + w * sin)
        # Calculate rotation matrix for rotating the cropped image around its center
        #(h, w) = im.shape[:2]
        center = (w / 2, h / 2)#rotate from the center
        ...
        debugimg = cropped.copy()
        debugimg[debugimg == 1] = 255
        if e is False:
            e = (np.nan,) * 10
            message = "E: eyes not detected!"
        else:
            e = (
            e[0][0]#[::-1]#reverse x and y since cv2.fitellipse uses y,x rather than x,y
            + e[0][1]#[::-1]
            + (-e[0][2],)
            + e[1][0]#[::-1]
            + e[1][1]#[::-1]
            ...
            c_x_e0 = e[0] # ellipse center x and y
            c_y_e0 = e[1]   # ellipse center x and y
            major_e0 = e[2] / scale  # major and minor axis
            minor_e0 = e[3] / scale# major and minor axis
            th_e0 = e[4]  # eye angle from tracked eclipse
            c_x_e1 = e[5]    # ellipse center x and y
            c_y_e1 = e[6]   # ellipse center x and y
            major_e1 = e[7] / scale # major and minor axis
            minor_e1 = e[8] / scale # major and minor axis
            th_e1 = e[9]  # eye angle from tracked eclipse
            # angle is local since the image was already rotaeted to heading direction
            # however, we need to calculate the real x and y based on window position
            #@chatgpt
            if c_x_e0 != 0 and c_y_e0 != 0 and c_x_e1 != 0 and c_y_e1 != 0:
                e0_xy_after = np.array([c_x_e0, c_y_e0, 1.0])
                c_x_e0_after, c_y_e0_after = (M_inv @ e0_xy_after)[:2]
                e1_xy_after = np.array([c_x_e1, c_y_e1, 1.0])
                c_x_e1_after, c_y_e1_after = (M_inv @ e1_xy_after)[:2]
                # map back to original full image coordinates
                c_x_e0_after, c_y_e0_after = (
                     wnd_pos[1]  + c_x_e0_after/scale ,  # X: column direction
                     wnd_pos[0] + c_y_e0_after/scale # Y: row direction
                )
                c_x_e1_after, c_y_e1_after = (
                    wnd_pos[1] + c_x_e1_after/scale ,  # X: column direction
                    wnd_pos[0] + c_y_e1_after/scale   # Y: row direction
                )

                e = (c_x_e0_after, c_y_e0_after, major_e0, minor_e0, th_e0,
                    c_x_e1_after, c_y_e1_after, major_e1, minor_e1, th_e1)
                if (debugimg.shape[1] > c_x_e0 > 0 and debugimg.shape[0] > c_y_e0 > 0
                    and debugimg.shape[1] > c_x_e1 > 0 and debugimg.shape[0] >  c_y_e1 > 0):
                    debugimg[int(c_y_e0), int(c_x_e0)] = 150
                    debugimg[int(c_y_e1), int(c_x_e1)] = 150
            else:
                e = [np.nan] * 10

        return message, e, debugimg
    ...
    def _process(
        ...
        elif self.set_diagnostic == 'eye tracking roi':
            try:
                self.diagnostic_image = eye_cropped
            except:
                self.diagnostic_image = bg
