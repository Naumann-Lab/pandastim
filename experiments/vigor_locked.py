...
from pandastim.behavior.Protocol import  BrukerClosedLoopProtocol, CenterClickTestingProtocol
from pandastim.behavior.Tracking import stytra_container
from pandastim.buddies.stimulus_buddies import StimulusBuddy, BrukerBuddy
...
def wrapper(protocol, stimulus_dataframe_path, ports, params, parameter_path):
    ...
    buddy = BrukerBuddy(comms = ports,
                             params_path =parameter_path,
                              protocol = protocol,
                              stimuli = stimulus_dataframe)  # start with pausing until Go, get stytra position output
    pstim = stimulus.BrukerStimulus(buddy=buddy, params_path=parameter_path, buddy_port = ports['buddy_stimulus_socket'])
...
if __name__ == '__main__':
        ...
        if params['pstim']:
               stimulus_process = mp.Process(target=wrapper, args=(BrukerClosedLoopProtocol, stimulus_path, _ports, params, parameter_path))
