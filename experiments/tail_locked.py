...
stimulus_path = Path(sys.executable).parents[0].joinpath(r'Lib\site-packages\pandastim\resources\protocols\tail_test.hdf')
...
def wrapper(protocol, stimulus_dataframe_path, ports, params, parameter_path):
    ...
    pstim = stimulus.TailLockedStimulus(buddy=stytraBuddy, params_path=parameter_path)#, buddy_port = ports['buddy_stimulus_socket'])
...
if __name__ == '__main__':
        ...
        if params['pstim']:
               stimulus_process = mp.Process(target=wrapper, args=(TailLockedProtocol, stimulus_path, _ports, params, parameter_path))
